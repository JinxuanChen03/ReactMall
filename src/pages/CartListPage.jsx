...
const CartListPage = () => {
    ...
    useEffect(() => {
        const fetchCartData = async () => {
            ...
            if (currentUser) {
                const userCarts = service.cart.getCartByUserId(currentUser.id);
                console.log('用户购物车数据:', userCarts); // 添加日志检查用户购物车数据

                const cartsWithDetails = userCarts.map(cart => {
                    const good = service.good.getGoodById(cart.goodId);
                ...
                }).filter(cart => cart !== null); // 过滤掉未找到商品的购物车项
    ...
    const handleRemove = (id) => {
        ...
        if (currentUser) {
            ...
            if (cartItem) {
                service.cart.removeFromCart(currentUser.id, cartItem.goodId);
